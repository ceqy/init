# 生产环境部署检查清单

## 📋 部署前检查（Pre-Deployment）

### 1. 基础设施准备

#### 1.1 服务器资源
- [ ] CPU: 至少 4 核（推荐 8 核）
- [ ] 内存: 至少 16GB（推荐 32GB）
- [ ] 磁盘: 至少 100GB SSD（推荐 500GB）
- [ ] 网络: 至少 1Gbps 带宽

#### 1.2 操作系统
- [ ] Linux 内核版本 >= 4.15
- [ ] Docker 版本 >= 20.10
- [ ] Docker Compose 版本 >= 2.0
- [ ] 时区配置正确（UTC 或本地时区）
- [ ] NTP 时间同步已启用

#### 1.3 网络配置
- [ ] 防火墙规则已配置
- [ ] 必要端口已开放（8080, 50051, 8500, 9090, 3001）
- [ ] DNS 解析正常
- [ ] 负载均衡器已配置（如果使用）

### 2. 安全配置

#### 2.1 TLS/SSL
- [ ] CA 证书已生成
- [ ] 服务证书已生成并签名
- [ ] 证书有效期检查（至少 30 天）
- [ ] 证书权限设置正确（私钥 600）
- [ ] mTLS 双向认证已启用
- [ ] 证书自动轮换已配置

#### 2.2 认证授权
- [ ] JWT_SECRET 已设置（至少 32 字符）
- [ ] 数据库密码已修改（不使用默认密码）
- [ ] Redis 密码已设置
- [ ] Consul ACL 已启用
- [ ] Envoy Admin 接口访问控制已配置

#### 2.3 网络安全
- [ ] 所有服务间通信已加密
- [ ] 敏感端口不对外暴露
- [ ] 防火墙规则最小化原则
- [ ] DDoS 防护已启用
- [ ] WAF（Web 应用防火墙）已配置

### 3. 数据持久化

#### 3.1 数据库
- [ ] PostgreSQL 数据目录已挂载到持久化存储
- [ ] 数据库连接池配置合理
- [ ] 慢查询日志已启用
- [ ] 数据库备份策略已配置
- [ ] 主从复制已配置（如果需要）

#### 3.2 备份策略
- [ ] 自动备份脚本已部署
- [ ] 备份 Cron 任务已配置
- [ ] 备份文件验证机制已实施
- [ ] 备份保留策略已设置（默认 30 天）
- [ ] 远程备份存储已配置（S3/OSS）
- [ ] 备份恢复流程已测试

#### 3.3 其他存储
- [ ] Redis 持久化已启用（AOF + RDB）
- [ ] Kafka 数据目录已持久化
- [ ] ClickHouse 数据目录已持久化
- [ ] Consul 数据目录已持久化

### 4. 监控与告警

#### 4.1 Prometheus
- [ ] Prometheus 已部署
- [ ] 所有服务指标已采集
- [ ] 数据保留期已配置（默认 15 天）
- [ ] 存储空间充足

#### 4.2 Grafana
- [ ] Grafana 已部署
- [ ] 数据源已配置
- [ ] 仪表盘已导入
- [ ] 用户权限已设置

#### 4.3 Alertmanager
- [ ] Alertmanager 已部署
- [ ] 告警规则已配置
- [ ] 通知渠道已测试（邮件/Slack/钉钉）
- [ ] 告警路由规则已设置
- [ ] 告警抑制规则已配置

#### 4.4 日志聚合
- [ ] 日志收集系统已部署（ELK/Loki）
- [ ] 所有服务日志已接入
- [ ] 日志保留策略已设置
- [ ] 日志查询权限已配置

### 5. 高可用配置

#### 5.1 Consul 集群
- [ ] Consul 集群节点数 >= 3
- [ ] Consul Leader 选举正常
- [ ] Consul 健康检查已配置
- [ ] Consul 备份已配置

#### 5.2 负载均衡
- [ ] Envoy 负载均衡策略已配置
- [ ] 健康检查间隔合理（10-30 秒）
- [ ] 熔断器阈值已设置
- [ ] 重试策略已配置

#### 5.3 故障转移
- [ ] 服务多实例部署（至少 2 个）
- [ ] 自动故障转移已测试
- [ ] 数据库主从切换已测试
- [ ] 灾难恢复计划已制定

### 6. 性能优化

#### 6.1 资源限制
- [ ] 容器 CPU 限制已设置
- [ ] 容器内存限制已设置
- [ ] 数据库连接池大小合理
- [ ] Envoy 连接池配置优化

#### 6.2 缓存配置
- [ ] Redis 缓存策略已优化
- [ ] 缓存过期时间合理
- [ ] 缓存预热机制已实施

#### 6.3 性能测试
- [ ] 负载测试已完成
- [ ] 压力测试已完成
- [ ] 性能基线已建立
- [ ] 瓶颈已识别并优化

### 7. 配置管理

#### 7.1 环境变量
- [ ] .env 文件已配置
- [ ] 敏感信息已加密存储
- [ ] 环境变量文档已更新

#### 7.2 配置文件
- [ ] Envoy 配置已优化
- [ ] Consul 配置已优化
- [ ] Prometheus 配置已优化
- [ ] 所有配置文件已版本控制

### 8. 文档与流程

#### 8.1 运维文档
- [ ] 部署文档已完善
- [ ] 故障排查手册已编写
- [ ] 运维 Runbook 已准备
- [ ] API 文档已更新

#### 8.2 应急预案
- [ ] 灾难恢复计划已制定
- [ ] 应急联系人列表已更新
- [ ] 回滚流程已文档化
- [ ] 应急演练已完成

---

## 🚀 部署执行（Deployment）

### 1. 部署前准备
```bash
# 1. 生成证书
cd /path/to/project
./scripts/generate-certs.sh

# 2. 配置环境变量
cp .env.example .env.production
vim .env.production  # 修改所有配置

# 3. 验证配置
docker-compose -f deploy/docker/docker-compose.envoy.yml config

# 4. 拉取镜像
docker-compose -f deploy/docker/docker-compose.envoy.yml pull
```

### 2. 数据库初始化
```bash
# 1. 启动数据库
docker-compose -f deploy/docker/docker-compose.envoy.yml up -d postgres

# 2. 等待数据库就绪
until docker exec postgres pg_isready -U postgres; do sleep 1; done

# 3. 运行迁移
docker exec postgres psql -U postgres -d cuba -f /migrations/init.sql

# 4. 验证数据库
docker exec postgres psql -U postgres -d cuba -c "\dt"
```

### 3. 启动服务
```bash
# 1. 启动基础设施
docker-compose -f deploy/docker/docker-compose.envoy.yml up -d \
    consul postgres redis kafka clickhouse

# 2. 等待基础设施就绪
sleep 30

# 3. 启动应用服务
docker-compose -f deploy/docker/docker-compose.envoy.yml up -d \
    iam-access iam-access-envoy gateway gateway-envoy

# 4. 启动监控服务
docker-compose -f deploy/docker/docker-compose.envoy.yml up -d \
    prometheus grafana alertmanager
```

### 4. 验证部署
```bash
# 1. 检查所有容器状态
docker-compose -f deploy/docker/docker-compose.envoy.yml ps

# 2. 检查 Consul 服务注册
curl http://localhost:8500/v1/catalog/services

# 3. 检查 Envoy 集群状态
curl http://localhost:9901/clusters | grep iam_cluster

# 4. 测试 API
curl http://localhost:8080/health

# 5. 检查监控指标
curl http://localhost:9090/api/v1/targets
```

---

## ✅ 部署后检查（Post-Deployment）

### 1. 功能验证
- [ ] 健康检查端点正常
- [ ] 用户登录功能正常
- [ ] API 调用正常
- [ ] WebSocket 连接正常
- [ ] 数据库读写正常

### 2. 监控验证
- [ ] Prometheus 采集指标正常
- [ ] Grafana 仪表盘显示正常
- [ ] Alertmanager 告警规则生效
- [ ] 日志收集正常

### 3. 性能验证
- [ ] API 响应时间 < 200ms (P99)
- [ ] 数据库查询时间 < 100ms (P99)
- [ ] CPU 使用率 < 70%
- [ ] 内存使用率 < 80%

### 4. 安全验证
- [ ] TLS 证书有效
- [ ] 认证授权正常
- [ ] 敏感端口不可外部访问
- [ ] 安全扫描无高危漏洞

### 5. 备份验证
- [ ] 自动备份任务运行正常
- [ ] 备份文件完整性验证通过
- [ ] 备份恢复流程测试通过

---

## 🔄 持续运维（Operations）

### 日常检查（每天）
- [ ] 检查服务健康状态
- [ ] 查看告警信息
- [ ] 检查磁盘空间
- [ ] 查看错误日志

### 周检查（每周）
- [ ] 检查备份完整性
- [ ] 查看性能趋势
- [ ] 检查证书有效期
- [ ] 更新安全补丁

### 月检查（每月）
- [ ] 执行备份恢复演练
- [ ] 审查告警规则
- [ ] 容量规划评估
- [ ] 安全审计

---

## 📞 应急联系

| 角色 | 姓名 | 电话 | 邮箱 | 职责 |
|------|------|------|------|------|
| 运维负责人 | - | - | ops@erp.com | 整体协调 |
| DBA | - | - | dba@erp.com | 数据库问题 |
| 平台工程师 | - | - | platform@erp.com | 基础设施 |
| 安全工程师 | - | - | security@erp.com | 安全事件 |

---

## 📚 相关文档

- [部署指南](ENVOY_DEPLOYMENT_GUIDE.md)
- [TLS 配置指南](TLS_CONFIGURATION_GUIDE.md)
- [灾难恢复手册](DISASTER_RECOVERY.md)
- [故障排查手册](TROUBLESHOOTING.md)
- [性能调优指南](PERFORMANCE_TUNING.md)
