...
//! mf-eng Service - Engineering Management
...
pub const FILE_DESCRIPTOR_SET: &[u8] = tonic::include_file_descriptor_set!("mf_eng_descriptor");

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    run_server("config", |infra: Infrastructure, mut server| async move {
        info!("Initializing mf-eng Service...");
