...
impl<R: RoleRepository, UR: UserRoleRepository> RoleQueryHandler<R, UR> {
    pub fn new(role_repo: Arc<R>, user_role_repo: Arc<UR>) -> Self {
        Self { role_repo, user_role_repo }
    }

    /// 获取角色详情
    pub async fn handle_get(&self, query: GetRoleQuery) -> AppResult<Role> {
        let role_id: RoleId = query.role_id.parse()
    ...
    pub async fn handle_get_by_code(&self, query: GetRoleByCodeQuery) -> AppResult<Role> {
        self.role_repo.find_by_code(&query.tenant_id, &query.code)
            .await?
            .ok_or_else(|| AppError::not_found("Role not found"))
    }
    ...
    pub async fn handle_list(&self, query: ListRolesQuery) -> AppResult<RoleListResult> {
        let (roles, total) = self.role_repo
    ...
    /// 获取用户权限
    pub async fn handle_get_user_permissions(&self, query: GetUserPermissionsQuery) -> AppResult<Vec<Permission>> {
        self.user_role_repo
    ...
    /// 检查用户是否有某个权限
    pub async fn handle_check_user_permission(&self, query: CheckUserPermissionQuery) -> AppResult<bool> {
        self.user_role_repo
