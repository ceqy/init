...
pub struct RedisEventPublisher {
    client: Client,
    base_channel: String, // fallback if needed, or prefix?
}
...
impl RedisEventPublisher {
    pub fn new(redis_url: &str, base_channel: &str) -> Result<Self, redis::RedisError> {
        ...
        Ok(Self {
            client,
            base_channel: base_channel.to_string(),
        })
    }
}
...
impl EventPublisher for RedisEventPublisher {
    ...
    async fn publish_batch<E: Serialize + Send + Sync>(&self, topic: &str, events: &[E]) -> AppResult<()> {
        let mut conn = self.client.get_multiplexed_async_connection().await
            .map_err(|e| cuba_errors::AppError::internal(format!("Redis connection failed: {}", e)))?;
        ...
        for event in events {
            ...
            if let Err(e) = conn.publish::<_, _, ()>(topic, payload).await {
                ...
                return Err(cuba_errors::AppError::internal(format!("Redis batch publish failed: {}", e)));
            }
        }
        Ok(())
    }
