...
mod tests {
    ...
    fn test_create_tenant() {
        let result = Tenant::new("test-tenant".to_string(), "Test Tenant".to_string());
        assert!(result.is_ok());
    ...
    #[test]
    fn test_invalid_tenant_name() {
        let result = Tenant::new("Test Tenant".to_string(), "Test Tenant".to_string());
        assert!(result.is_err());

        let result = Tenant::new("-invalid".to_string(), "Invalid".to_string());
        assert!(result.is_err());
    }

    #[test]
    fn test_activate_tenant() {
        let mut tenant = Tenant::new("test".to_string(), "Test".to_string()).unwrap();
        assert!(tenant.activate().is_ok());
    ...
    fn test_suspend_tenant() {
        ...
        assert!(tenant.suspend("Payment failed".to_string()).is_ok());
    ...
    fn test_extend_subscription() {
        ...
        tenant.extend_subscription(365);
        assert!(tenant.subscription_ends_at.is_some());
    }

    #[test]
    fn test_is_available() {
        let mut tenant = Tenant::new("test".to_string(), "Test".to_string()).unwrap();
        assert!(tenant.is_available()); // Trial is available

        tenant.activate().unwrap();
        assert!(tenant.is_available()); // Active without expiry

        tenant.suspend("Test".to_string()).unwrap();
        assert!(!tenant.is_available()); // Suspended is not available
    }
    ...
    fn test_set_domain() {
        ...
        assert!(tenant.set_domain("example.com".to_string()).is_ok());
        assert_eq!(tenant.domain, Some("example.com".to_string()));

        assert!(tenant.set_domain("invalid".to_string()).is_err());
    }

    #[test]
    fn test_update_password_policy() {
        let mut tenant = Tenant::new("test".to_string(), "Test".to_string()).unwrap();
        ...
        tenant.update_password_policy(policy.clone());
        assert_eq!(tenant.settings.password_policy.min_length, 12);
        assert!(tenant.settings.password_policy.require_2fa);
    }
}
