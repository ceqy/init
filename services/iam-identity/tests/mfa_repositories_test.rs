...
use iam_identity::domain::repositories::auth::{BackupCodeRepository, WebAuthnCredentialRepository};
...
async fn test_backup_code_lifecycle() {
    ...
    repo.save(&backup_code).await.expect("Failed to save backup code");

    // 2. Find Available
    let codes = repo.find_available_by_user_id(&user_id, &tenant_id).await.expect("Failed to find codes");
    ...
    // 4. Verify used
    let available_after = repo.find_available_by_user_id(&user_id, &tenant_id).await.expect("Failed to find");
    assert!(available_after.is_empty());
    
    // 5. Find by ID (should still exist)
    let found = repo.find_by_id(&code_to_use.id, &tenant_id).await.expect("Failed to find by id");
    assert!(found.is_some());
    assert!(found.unwrap().used);
    
    // 6. Delete
    repo.delete_by_user_id(&user_id, &tenant_id).await.expect("Failed to delete");
    let found_after_delete = repo.find_by_id(&code_to_use.id, &tenant_id).await.expect("Failed to find");
...
async fn test_webauthn_credential_lifecycle() {
    ...
    // 1. Save
    repo.save(&credential).await.expect("Failed to save credential");

    // 2. Find by User
    let creds = repo.find_by_user_id(&user_id, &tenant_id).await.expect("Failed to find by user");
    ...
    let found = repo.find_by_id(&credential.id, &tenant_id).await.expect("Failed to find by id");
    assert!(found.is_some());

    // 4. Update counter
    let mut cred_to_update = found.unwrap();
    cred_to_update.update_counter(10);
    repo.update(&cred_to_update).await.expect("Failed to update");

    let updated = repo.find_by_id(&credential.id, &tenant_id).await.unwrap().unwrap();
    assert_eq!(updated.counter, 10);
    
    // 5. Delete
    repo.delete(&credential.id, &tenant_id).await.expect("Failed to delete");
    
    let deleted = repo.find_by_id(&credential.id, &tenant_id).await.expect("Failed to find");
...
async fn test_tenant_isolation_2fa() {
    ...
    let found_by_b = repo.find_by_id(&backup_code.id, &tenant_b).await.expect("Query failed");
    assert!(found_by_b.is_none());
    
    // Tenant A find User A's code
    let found_by_a = repo.find_by_id(&backup_code.id, &tenant_a).await.expect("Query failed");
