...
async fn create_fixtures(
    ...
    client_repo.save(&client).await.expect("Failed to save fixture client");
...
async fn test_authorization_code_lifecycle() {
    ...
    // 1. Save
    repo.save(&auth_code).await.expect("Failed to save auth code");

    // 2. Find by code
    let found = repo.find_by_code(&code_str, &tenant_id).await.expect("Failed to find");
    ...
    // 3. Delete
    repo.delete(&code_str, &tenant_id).await.expect("Failed to delete");
    
    // 4. Verify deleted
    let found_after = repo.find_by_code(&code_str, &tenant_id).await.expect("Failed to find");
...
async fn test_access_token_lifecycle() {
    ...
    // 1. Save
    repo.save(&access_token).await.expect("Failed to save access token");

    // 2. Find
    let found = repo.find_by_token(&token_str, &tenant_id).await.expect("Failed to find");
    assert!(found.is_some());
    ...
    // 4. Verify revoked
    let found_revoked = repo.find_by_token(&token_str, &tenant_id).await.expect("Failed to find");
...
async fn test_refresh_token_lifecycle() {
    ...
    at_repo.save(&access_token).await.expect("Failed to save prerequisite access token");
    ...
    // 1. Save
    repo.save(&refresh_token).await.expect("Failed to save refresh token");
