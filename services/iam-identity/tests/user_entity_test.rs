...
use iam_identity::domain::value_objects::{Email, Username, HashedPassword};


/// 测试辅助：创建测试用户
fn create_test_user(tenant_id: TenantId) -> User {
    let username = Username::new(format!("testuser_{}", uuid::Uuid::new_v4().to_string()[..8].to_string()))
        .expect("Valid username");
    let email = Email::new(format!("test_{}@example.com", uuid::Uuid::new_v4().to_string()[..8].to_string()))
        .expect("Valid email");
    let password_hash = HashedPassword::from_hash("$argon2id$v=19$m=65536,t=3,p=4$test_hash".to_string());
